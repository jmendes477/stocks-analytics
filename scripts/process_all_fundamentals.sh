...
# process_all_fundamentals.sh
# Usage: ./scripts/process_all_fundamentals.sh [--symbols-file PATH] [--parallel N] [--skip-processed] [--retry-failed]
#
# Reads a symbols file (one ticker per line) and runs `node scripts/process-fundamentals.cjs <SYMBOL>` for each.
# Features:
# - Parallel execution via xargs -P (default 4)
# - Optional skip of already-processed symbols (requires psql and DATABASE_URL/POSTGRES_URL env)
# - Logging to logs/process_fundamentals.log and logs/process_fundamentals.err
# - Failed symbols appended to logs/fundamentals_failed.txt for later retry
...
Options:
  ...
  --skip-processed     Skip symbols that already exist in fundamentals_latest (requires psql and DATABASE_URL/POSTGRES_URL)
  --retry-failed        Retry only symbols recorded in logs/fundamentals_failed.txt
...
# Capture absolute node path used for the main check to ensure worker processes use the same binary
...
mkdir -p "$LOGDIR"
PROCESS_LOG="$LOGDIR/process_fundamentals.log"
ERROR_LOG="$LOGDIR/process_fundamentals.err"
FAILED_LIST="$LOGDIR/fundamentals_failed.txt"
: >"$PROCESS_LOG"
: >"$ERROR_LOG"
: >"$FAILED_LIST"
...
if [[ "$SKIP_PROCESSED" -eq 1 ]]; then
  ...
  eval "psql '$PSQL_CONN' -t -A -c \"SELECT symbol FROM fundamentals_latest\"" >"$PROCESSED_TMP" || true
...
# Worker command for xargs - runs process-fundamentals and logs failures
worker_cmd() {
  symbol="$1"
  if "$NODE_BIN" "$SCRIPTDIR/process-fundamentals.cjs" "$symbol" >>"$PROCESS_LOG" 2>>"$ERROR_LOG"; then
    echo "OK: $symbol"
    return 0
  else
